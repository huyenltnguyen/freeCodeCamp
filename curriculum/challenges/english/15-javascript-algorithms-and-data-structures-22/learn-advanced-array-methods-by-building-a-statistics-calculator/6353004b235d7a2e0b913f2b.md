...
```js
const code = getStandardDeviation.toString();
const matches = code.match(/(?:Math.pow\(\s*variance\s*,)\s*((\d|\.).*)\s*(?:\))/);
const exponent = matches ? matches[1] : null;

assert.isTrue(exponent !== null);
assert.equal(eval(exponent), 0.5);
```
