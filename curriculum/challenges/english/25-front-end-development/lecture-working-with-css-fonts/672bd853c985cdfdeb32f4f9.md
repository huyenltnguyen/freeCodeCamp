...
Here's an example of how to apply an `X` and `Y` offset on a shadow. You apply the `text-shadow` property with an x-offset of `3px` and a y-offset of `2px`:
...
In the browser, the text and the shadow will look similar but you can also customize the color of the shadow by specifying the value before or after the offset. Let's set the shadow color. You will use a hexadecimal color here but you can use any valid color format. 
