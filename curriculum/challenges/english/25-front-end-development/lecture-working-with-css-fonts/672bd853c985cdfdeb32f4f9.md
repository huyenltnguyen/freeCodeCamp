...
Here's an example of how to apply an `X` and `Y` offset on a shadow. In this example, you apply the `text-shadow` property with an x-offset of `3px` and a y-offset of `2px`:
...
In the browser, the text and the shadow will look similar but you can also customize the color of the shadow by specifying the value before or after the offset. Let's set the shadow color. This example will use a hexadecimal color but you can use any valid color format. 
...
You are writing the color after the offset but you can also write the color before the offset if you prefer. It's equivalent:
...
Now the shadow will move left and up in relation to the text. Great. But the shadow is not looking very nice, because it looks exactly like the original text but placed behind it. To make it look nicer, you need to add a third value, the blur radius. This value is optional but makes the shadow look a lot smoother and more subtle. The default value of the radius blur is zero. The higher the value, the bigger the blur, which means that the shadow becomes lighter.

Here, you are setting the blur radius to `3px` and you're back to positive values for the offset:
...
Now it's starting to look like a shadow. It's more blurry and subtle, so you can focus on the main text while the shadow adds some style in the background. From here, you can tweak the values a little bit until you find a combination of offset, color, and blur radius that fits your needs.
